...
function svgFromCells(cells, size) {
  ...
  for (let gr = 0; gr < GH; gr++) for (let gc = 0; gc < GW; gc++) {
    ...
    // The host-colour bridge (`hostFaces`: edge-adjacent host-colour faces) ALWAYS applies -- a strict poke that fills
    // the pinch where the band crosses a seam, e.g. the run-off end O landing on an interior column seam, so the line
    // continues onto the base-colour part of the next tile (including a triangle's base-colour half) and runs off the
    // border. One exclusion: a vertex tile (NB or CC) whose LINE side -- the half along the seam it shares with the
    // host -- is NOT the host colour touches the host only at the point V, so poking its far host-coloured corner would
    // nub the line past V into that colour; the line must stop when it reaches the neighbour's colour, so that tile is
    // skipped (tiles sharing the seam away from V keep their poke -- they bridge a real edge, not a single point). When
    // pokeOK, ALSO draw over the perpendicular wedge tiles at V (cross-centre and neighbour, any colour) so the line
    // keeps full width through the foreign wedges flanking a centre vertex. The diagonal tile is never poked: the line
    // continues into it only where it is the accent colour (via the accent-face clip).
