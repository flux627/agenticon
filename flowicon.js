 ...
 *
 * A 4x2 grid of cells (solids, halves, corner blocks, diagonals, corner triangles) filled
 * in a shuffled order; at each cell we enumerate the legal tiles -- those that leave no
 * orphaned 1/4 square (every quarter shares a colour with a neighbour in-cell or across a
 * seam) -- and pick the most edge-contiguous, growing a continuous flow across the grid; a
 * rare deadlock is fixed by re-placing one neighbour. A recolour pass then remaps the icon
...
const KIND_W = { Q: 0.50, T: 0.32 };              // kind preference among legal candidates
const ALL_MASKS = [
...
const SOLIDS = PALETTE.map((x) => ({ kind: "Q", data: [false, false, false, false], fg: x, bg: x }));  // always-legal floor
